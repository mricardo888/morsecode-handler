...
class LanguageNotFoundError(MorseCodeError):
    ...
    def __init__(self, language: str, available: Optional[list[str]] = None):
        ...
        msg = f"Language '{language}' not found. Available: {', '.join(self.available) or 'None'}."  # noqa: E501
        super().__init__(msg)


class UnknownCharacterError(MorseCodeError):
    """Raised when encoding encounters a character not present in the selected language."""  # noqa: E501
    ...
    def __init__(self, char: str, position: int, language: str):
        ...
        self.language = language
        super().__init__(
            f"Unknown character {char!r} at position {position} for language '{language}'."  # noqa: E501
        )


class UnknownMorseCodeError(MorseCodeError):
    """Raised when decoding encounters a token not present in the selected language."""  # noqa: E501
    ...
    def __init__(self, token: str, index: int, language: str):
        ...
        self.language = language
        super().__init__(
            f"Unknown Morse token {token!r} at index {index} for language '{language}'."  # noqa: E501
        )
